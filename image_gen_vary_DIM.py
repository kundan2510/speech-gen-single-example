...
GRAD_CLIP = 1.0
OUTPUT_DIR = "/Tmp/kumarkun/generated_texture_BRICK_vary_DIM"
BATCH_SIZE = 1
...
filter_sizes = [201, 171, 151, 121, 101, 71, 51, 41, 31, 21]
expn = 0
...
for filter_size in filter_sizes:
	for DIM in DIMS:
		...
		expn += 1
		OTHER_INFO = "multiscale_fs_{}_dim_{}_exp_{}".format(filter_size, DIM, expn)
		params = []
		other_params = []
	...
	# filter_sizes = [3, 5, 11, 21]
		...
		current_filter = get_conv_2d_filter(
						(DIM, 1, filter_size,filter_size), 
						param_list = other_params, 
						masktype = None, 
						name = "filter_{}".format(filter_size)
						)

		curr_conv_out_input = T.nnet.conv2d(input_sound, current_filter, border_mode = 'half') # shape = (batch_size, 128, input_length, 1)
		curr_conv_out_output = T.nnet.conv2d(output_sound, current_filter, border_mode= 'half') # shape = (batch_size, 128, input_length, 1)
		...
		input_feature_map = T.nnet.relu(curr_conv_out_input)
		output_feature_map = T.nnet.relu(curr_conv_out_output)
